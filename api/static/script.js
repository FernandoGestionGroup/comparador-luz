...
const I18N = {
  ...
  ca: {
    ...
    "Cálculos de caràcter informatiu no vinculants.": "Càlculs de caràcter informatiu no vinculants.", "Fecha propuesta": "Data proposta", "Nombre del Asesor": "Nom de l'Assessor",
...
function getForm(){
  ...
  return {
    ...
    potencia_kw:n($('f_pot').value), dıas:n($('f_dias').value),
        ...
        ${users.map(u => `
          <tr>
            ...
            <td style="display:flex;gap:5px">
              <button class="btn btn-s btn-sm" onclick="openUserMod('${u.id}')">✎</button>
              ${isAdmin && u.id !== ST.user.id ? `<button class="btn btn-d btn-sm" onclick="delUser('${u.id}')">✕</button>` : ''}
            </td>
          </tr>
        `).join('')}
      </tbody>
  ...
  try {
    const r = await fetch('/api/usuarios',{
      ...
      body:JSON.stringify({
        ...
        id: ST.editUserId,
        nombre, email, password: pass, role
      })
    });
    const d = await r.json();
    if(d.ok) {
      ...
      sb('Error: ' + (d.error || 'No se pudo guardar'), 'err');
    }
  } catch(e) {
    sb('Error de conexión', 'err');
  }
}

async function delUser(id){
  if(!confirm('¿Estás seguro de que deseas eliminar este usuario?')) return;
  sb('Eliminando usuario...', 'load');
  try {
    const r = await fetch('/api/usuarios', {
      method:'POST',
      headers:{'Content-Type':'application/json'},
    ...
    if(d.ok) {
      await loadUsuarios();
      renderUsersTable();
      sb('Usuario eliminado ✓', 'ok');
    } else {
      sb('Error: ' + (d.error || 'No se pudo eliminar'), 'err');
    }
  } catch(e) {
    sb('Error de conexión', 'err');
  }
