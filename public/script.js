...
async function renderCmp(){
  ...
  try {
    const resp = await fetch('/api/calculate', {
      ...
      body: JSON.stringify({ invoice_data: d })
    });
    
    if (!resp.ok) throw new Error('Error al calcular comparativa');
    const res = await resp.json();
    ST._res = res;
    
    if (res.length === 0) {
      $('rankDiv').innerHTML = '<div class="empty">No hay ofertas compatibles con esta tarifa y potencia.</div>';
      return;
    }
...
// La lógica de cálculo se ha movido al backend para mayor seguridad y consistencia.
