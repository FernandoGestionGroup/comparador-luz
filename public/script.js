...
function uiTogIA(){
  ...
  if(p === 'auto') { fields.forEach(el => el.style.display = 'block'); }
  else { const targetId = 'ia_' + (p === 'groq' ? 'openai' : p); if($(targetId)) $(targetId).style.display = 'block'; }
  const mod = $('cfg_model');
  if(mod) {
    let d = 'Recomendado';
...
async function loadConfig(){ try{ const r=await fetch('/api/config'); ST.config=await r.json(); } catch(e){} }
...
async function loadComisiones(){ try{ const r=await fetch('/api/comisiones'); ST.comisiones=await r.json(); } catch(e){} }
...
async function saveComs(){
  ...
  const tramos = Array.from($('comTramosT').querySelectorAll('tr')).map(tr => {
    ...
    return { p_min: n(inp[0].value), p_max: n(inp[1].value), c_min: n(inp[2].value), c_max: n(inp[3].value), filtro: inp[4].value.trim(), valor: n(inp[5].value), tipo: inp[6].value };
  });
  ...
  sb('Comisiones guardadas ✓','ok'); renderComList();
}
...
function renderComList(){
  ...
  el.innerHTML = ST.comisiones.map(c => `<div class="com-item ${ST.selCom?.id === c.id ? 'active' : ''}" onclick="openComCo('${c.id}')">${c.comercializadora}<span style="font-size:9px;opacity:0.6">${c.tramos?.length || 0} tramos</span></div>`).join('');
}

async function delComCo(){
  if(!ST.selCom || !confirm('¿Eliminar esta empresa y todas sus reglas?')) return;
  ST.comisiones = ST.comisiones.filter(c => c.id !== ST.selCom.id); ST.selCom = null;
  await fetch('/api/comisiones',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(ST.comisiones)});
  renderComList(); $('comEditor').style.display = 'none'; $('comEditorEmpty').style.display = 'flex';
}

function openComCo(id){
  const c = ST.comisiones.find(x => x.id === id); if(!c) return;
  ST.selCom = c; $('comEditorEmpty').style.display = 'none'; $('comEditor').style.display = 'block';
  $('comCoName').textContent = c.comercializadora; $('comTramosT').innerHTML = '';
...
function addComTramo(t){
  ...
  tr.innerHTML = `<td><input type="number" step="0.01" value="${t.p_min}"></td><td><input type="number" step="0.01" value="${t.p_max}"></td><td><input type="number" step="1" value="${t.c_min}"></td><td><input type="number" step="1" value="${t.c_max}"></td><td><input type="text" value="${t.filtro || ''}"></td><td><input type="number" step="0.01" value="${t.valor}"></td><td><select><option value="fijo" ${t.tipo==='fijo'?'selected':''}>Fijo (€)</option><option value="variable" ${t.tipo==='variable'?'selected':''}>Variable (€/MWh)</option></select></td><td><button onclick="this.closest('tr').remove()">✕</button></td>`;
...
function buildTbls(){ $('potT').innerHTML=''; $('enT').innerHTML=''; $('lecT').innerHTML=''; PS.forEach(p=>{ addPotL(p); addEnL(p); addLecL(p); }); }
function addPotL(per){
  ...
  tr.innerHTML='<td class="lbl">'+p+'</td><td><input type="number" class="pk_in" value="0"></td><td><input type="number" class="pi_in" value="0"></td><td><button onclick="this.closest(\'tr\').remove()">✕</button></td>';
  $('potT').appendChild(tr);
}
function addEnL(per){
  const p=per||$('addEnSel').value; const id=enIdSeq++;
  const tr=document.createElement('tr'); tr.className='en-row'; tr.dataset.per=p;
  tr.innerHTML='<td class="lbl">'+p+'</td><td><input type="number" class="ek_in" value="0"></td><td><input type="number" class="ep_in" value="0"></td><td><button onclick="this.closest(\'tr\').remove()">✕</button></td>';
  $('enT').appendChild(tr);
}
function addLecL(per){
  ...
  tr.innerHTML='<td class="lbl">'+p+'</td><td><input type="number" class="lk_in" value="0"></td><td><button onclick="this.closest(\'tr\').remove()">✕</button></td>';
  $('lecT').appendChild(tr);
}
...
function getForm(){
  ...
  document.querySelectorAll('.pot-row').forEach(tr=>{ potArr.push({per:tr.dataset.per, kw:n(tr.querySelector('.pk_in').value), importe:n(tr.querySelector('.pi_in').value)}); });
  document.querySelectorAll('.en-row').forEach(tr=>{ enArr.push({per:tr.dataset.per, kwh:n(tr.querySelector('.ek_in').value), precio:n(tr.querySelector('.ep_in').value)}); });
  const lecByPer = {}; PS.forEach(p=>{ lecByPer[p]=0; });
  document.querySelectorAll('.lec-row').forEach(tr=>{ const k=n(tr.querySelector('.lk_in').value); lecByPer[tr.dataset.per]=k; });
  return { cliente:$('f_cli').value, cups:$('f_cups').value, comercializadora:$('f_com').value, direccion:$('f_dir').value, cp:$('f_cp').value, potencia_kw:n($('f_pot').value), dias:n($('f_dias').value), total_factura:n($('f_tot').value), iva_pct:n($('f_iva').value)||21, iee_pct:n($('f_iee').value)||5.1, iva_act:n($('f_iva_act').value), iee_act:n($('f_iee_act').value), dto_en_act_pct:n($('f_dto_en_act').value), reactiva:n($('f_rea').value), exceso_potencia:n($('f_exc').value), alquiler_equipos:n($('f_alq').value), bono_social:n($('f_bon').value), servicio:n($('f_ser').value), tarifa:$('f_tar').value, consumo_anual:n($('f_consumo_anual').value), tiene_autoconsumo:$('f_tiene_autocon').checked, autoconsumo_kwh:n($('f_aut_kwh').value), autoconsumo_precio_kwh:n($('f_aut_precio').value), pot_p:potArr, en_p:enArr, lec_by_per:lecByPer };
}
...
function calcOfr(d, o){
  const simByPer = d.lec_by_per || {}; let tPot=0, tEn=0;
  PS.forEach(p=>{ const pp=o['pp_'+p.toLowerCase()]||0; tPot+=d.potencia_kw*pp*d.dias; const ep=o['ep_'+p.toLowerCase()]||0; tEn+=(simByPer[p]||0)*ep; });
